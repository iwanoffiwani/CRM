  ...
  const submitHandler = e => {
    ...
    if (nextState.fields) 
      filter = {
        ...
        previousState: {
          ...filter.previousState,
          fields: previousState.fields.filter((change, index) => 
            change.value !== nextState.fields[index].value)
        },
        nextState: {
          ...filter.nextState,
          fields: nextState.fields.filter((change, index) => 
    ...
    .map((comment, index) => {
      if (comment.nextState) {
        ...
        if (comment.nextState.fields) {
          return (
            comment.nextState.fields.map((field, index) => 
              <ListItem key={index} className={classes.listItem}>
                <ListItemText
