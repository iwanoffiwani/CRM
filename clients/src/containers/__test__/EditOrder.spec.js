...
describe("Tests EditOrder component", () => {
  ...
  it("+++ renderer component", () => {
    const wrapper = shallow(
      <EditOrder {...props} {...mapStateToProps} {...mapDispatchToProps} />
    );
  ...
  it("+++ check for statuses in the order", () => {
    ...
    const { statuses } = mapStateToProps;
  ...
  it("+++ simulation of order status change", async () => {
    ...
    const nextStatus =
      name === statuses[0].name ? statuses[1].name : statuses[0].name;
    const nextStatusID =
      _id === statuses[0]._id ? statuses[1]._id : statuses[0]._id;
