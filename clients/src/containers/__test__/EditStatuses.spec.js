...
describe('Tests EditStatuses component', () => {
  ...
  it('+++ simulation of a click event on the status editing button', () => {
    ...
    expect(textFieldWrapper.props().value).toBe(mapStateToProps.statuses[0].name)
  ...
  it('+++ simulation of status name change', () => {
    ...
    textFieldWrapper.simulate('change', { target: { value: 'Some status name' } })
    expect(wrapper.find(TextField).at(0).props().value).toBe('Some status name')
