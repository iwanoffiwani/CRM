...
export const TableOrders = props => {
  ...
  const changeStatusHandler = (e, order) => {
    ...
    const {
      _id: orderID,
      status: { name: prevStatus },
      changes
    } = order; // Получаем id заявки, текущий статус заявки и массив изменений

    const { id: statusID } = e._targetInst.stateNode; // Получаем id статуса
  ...
  const otherColumns = props.columns.map((column, index) => (
    <StyledTableCell key={index} align="right" className={classes.tabelCell}>
      {column.name}
    </StyledTableCell>
  ));

  if (props.rows.length === 0) return <Preloader />;
  else
    return (
      <Layout>
        <Paper className={classes.root} component="section" elevation={7}>
          <div className={classes.wrapper}>
            <Table stickyHeader className={classes.table}>
              <TableHead>
                <TableRow>
                  <StyledTableCell className={classes.tabelCell}>
                    Имя
                  </StyledTableCell>
                  <StyledTableCell className={classes.tabelCell}>
                    Статус
              ...
              <TableBody>
                {(() => {
                  ...
                  const rows =
                    props.search.length === 0 ? props.rows : props.search;
        ...
        >
          ...
          <Divider />
          <EditOrder editOrder={state.edit.order} />
        </Drawer>
      </Layout>
    );
};
